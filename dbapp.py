...
def step2health():
    image_id = int(request.form.get('q1','')) #ラジオボタンのvalueの値を数値にして変数に格納。DBで使う
    conn = sqlite3.connect('vision.db')
    c = conn.cursor()
    c.execute("select path from images where imageid = ?",(image_id,))
    image_path = c.fetchone()
    step1 = image_path[0]
    c.execute("insert into userinput values(?,?,?,?,?,?,?,?)",(0,step1,0,0,0,0,0,0,))
    conn.commit()
    conn.close()
...
def step3family():
    image_id = int(request.form.get('q2','')) #ラジオボタンのvalueの値を数値にして変数に格納。DBで使う
    conn = sqlite3.connect('vision.db')
    c = conn.cursor()
    c.execute("select path from images where imageid = ?",(image_id,))
    image_path = c.fetchone()
    step2 = image_path[0]
    c.execute("update userinput set cate2 = ?",(step2,))
    conn.commit()
    conn.close()
...
def step4work():
    image_id = int(request.form.get('q3','')) #ラジオボタンのvalueの値を数値にして変数に格納。DBで使う
    conn = sqlite3.connect('vision.db')
    c = conn.cursor()
    c.execute("select path from images where imageid = ?",(image_id,))
    image_path = c.fetchone()
    step3 = image_path[0]
    c.execute("update userinput set cate3 = ?",(step3,))
    conn.commit()
    conn.close()
...
def step5hobby():
    image_id = int(request.form.get('q4','')) #ラジオボタンのvalueの値を数値にして変数に格納。DBで使う
    conn = sqlite3.connect('vision.db')
    c = conn.cursor()
    c.execute("select path from images where imageid = ?",(image_id,))
    image_path = c.fetchone()
    step4 = image_path[0]
    c.execute("update userinput set cate4 = ?",(step4,))
    conn.commit()
    conn.close()
...
def step6modele():
    image_id = int(request.form.get('q5','')) #ラジオボタンのvalueの値を数値にして変数に格納。DBで使う
    conn = sqlite3.connect('vision.db')
    c = conn.cursor()
    c.execute("select path from images where imageid = ?",(image_id,))
    image_path = c.fetchone()
    step5 = image_path[0]
    c.execute("update userinput set cate5 = ?",(step5,))
    conn.commit()
    conn.close()
...
def step7want():
    image_id = int(request.form.get('q6','')) #ラジオボタンのvalueの値を数値にして変数に格納。DBで使う
    conn = sqlite3.connect('vision.db')
    c = conn.cursor()
    c.execute("select path from images where imageid = ?",(image_id,))
    image_path = c.fetchone()
    step6 = image_path[0]
    c.execute("update userinput set cate6 = ?",(step6,))
    conn.commit()
    conn.close()
...
def finalpage():
    image_id = int(request.form.get('q7','')) #ラジオボタンのvalueの値を数値にして変数に格納。DBで使う
    conn = sqlite3.connect('vision.db')
    c = conn.cursor()
    c.execute("select path from images where imageid = ?",(image_id,))
    image_path = c.fetchone()
    step7 = image_path[0]
    c.execute("update userinput set cate7 = ?",(step7,))
    conn.commit()
    conn.close()
...
def visionBoard():
    name = request.form.get("user_name") #DBで使う
    conn = sqlite3.connect('vision.db')
    c = conn.cursor()
    c.execute("update userinput set name = ?",(name,))
    c.execute("select * from userinput where name = ?",(name,))
    result = c.fetchone()
    name = result[0]
    step1 = result[1]
    step2 = result[2]
    ...
    conn.commit()
    conn.close()
    return render_template("/visionBoard.html", name = name, step1 = step1, step2 = step2, step3 = step3, step4 = step4, step5 = step5, step6 = step6, step7 = step7)
